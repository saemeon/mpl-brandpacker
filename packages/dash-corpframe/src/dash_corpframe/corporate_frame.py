...
"""Corporate frame renderer for dash-capture.

Wraps a captured image with a header (title + underlined accent, subtitle)
and footer (footnotes, sources) using matplotlib.
"""
...
def corporate_renderer(
    ...
    ----------
    _target :
        File-like object to write to (injected by dash-capture).
    _snapshot_img :
        Callable returning raw PNG bytes (injected by dash-capture).
    title :
